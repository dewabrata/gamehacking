...
map_img = cv.imread('map.jpg', cv.IMREAD_UNCHANGED)
impostor_img = cv.imread('character2.jpg', cv.IMREAD_UNCHANGED)
...
result = cv.matchTemplate(map_img, impostor_img, cv.TM_CCOEFF_NORMED)
...
# that most closely matches our impostor image. The max value gives an indication
# of how similar that find is to the original impostor, where 1 is perfect and -1
# is exact opposite.
...
if max_val >= threshold:
    print('Found impostor.')

    # Get the size of the impostor image. With OpenCV images, you can get the dimensions 
    # via the shape property. It returns a tuple of the number of rows, columns, and 
    # channels (if the image is color):
    impostor_w = impostor_img.shape[1]
    impostor_h = impostor_img.shape[0]

    # Calculate the bottom right corner of the rectangle to draw
    top_left = max_loc
    bottom_right = (top_left[0] + impostor_w, top_left[1] + impostor_h)

    # Draw a rectangle on our screenshot to highlight where we found the impostor.
    ...
    cv.imwrite('result.jpg', map_img)

else:
    print('Impostor not found.')
